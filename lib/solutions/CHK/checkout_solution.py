...
def checkout(skus) -> int:
    ...
    if len(skus) > 1 and skus.isupper():
        ...
        for item in items:
            ...
            # elif item in ["S", "T", "X", "Y", "Z"] and item_count == skus.count("T") >= 0 or item_count == skus.count(
            #         "X") or item_count == skus.count("Y") >= 1 or item_count == skus.count("Z") >= 1 and len(set(skus)) == 1:
            #     return item_count * 15
